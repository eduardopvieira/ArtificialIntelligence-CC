...
public class MLP {
    ...
    private double sigmoid(double x) {
        return 1 / (1 + Math.exp(-x)); // Implementação da função sigmoid
    }

    // Derivada da função sigmoid, usada para calcular os ajustes dos pesos durante
    // a retropropagação
    private double derivadaSigmoid(double x) {
        return x * (1 - x); // Derivada da função sigmoid em termos de sua própria saída
    }
    ...
    // Método para a propagação para frente (feedforward)
    public double[] propagacaoFrente(double[] entradas) {
        ...
        for (int i = 0; i < biasOculta.length; i++) {
            ...
            for (int j = 0; j < entradas.length; j++) {
                soma += entradas[j] * pesosEntradaOculta[j][i]; // Soma ponderada das entradas e pesos
            }
    ...
    public void retropropagacao(double[] entradas, double[] saidaEsperada, double taxaAprendizado) {
        ...
        for (int i = 0; i < camadaSaida.length; i++) {
            errosSaida[i] = saidaEsperada[i] - camadaSaida[i]; // Erro = saída esperada - saída atual
            deltasSaida[i] = errosSaida[i] * derivadaSigmoid(camadaSaida[i]); // Delta = erro * derivada sigmoid
        }
        ...
        for (int i = 0; i < camadaOculta.length; i++) {
            ...
            errosOculta[i] = soma;
            deltasOculta[i] = errosOculta[i] * derivadaSigmoid(camadaOculta[i]); // Delta = erro * derivada sigmoid
        }
